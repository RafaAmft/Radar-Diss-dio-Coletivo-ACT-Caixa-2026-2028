...
payload = {
    ...
    'classesProcessuais': [{'codFase': 'DCG', 'desFase': 'Dissídio Coletivo de Greve'}, {'codFase': 'DC', 'desFase': 'Dissídio Coletivo'}],
...
print("Iniciando coleta de todos os DC/DCG na SDC do TST (2016-2026)...", flush=True)
...
while True:
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"Erro na requisição offset {current_offset}: status {r.status_code}", flush=True)
            break
        data = r.json()
        total_expected = data.get('totalRegistros', total_expected)
        regs = data.get('registros', [])
        if not regs:
            print(f"Nenhum registro retornado no offset {current_offset}. Fim da paginação.", flush=True)
            break
        for item in regs:
            all_records.append(item.get('registro', {}))
        print(f"Coletados {len(all_records)} / {total_expected} registros (offset={current_offset})...", flush=True)
...
output_path = 'scraping_dissidio_coletivo_greve/sdc_all_dc_dcg.json'
with open(output_path, 'w', encoding='utf-8') as f:
    json.dump(all_records, f, ensure_ascii=False, indent=2)

print(f"Salvo com sucesso! Total de registros coletados: {len(all_records)} em {output_path}", flush=True)
